...
namespace Assembler.Pseudoinstructions.LoadPseudoinstructions {
    ...
    /// AddwI $rDest, #value16
    /// </summary>
    class LbramoiPseudoinstruction : PseudoinstructionBase {
        public override PseudoinstructionType Type => PseudoinstructionType.LbramoI;
        ...
        protected override IEnumerable<IEnumerable<BitArray>> ParseInner(TokenReader reader) {
            ...
            if (DoesDestRegisterViolateDefaultRestrictions(rDestToken)) {
                yield return InstructionEncoder.Encode(McInstructionType.Lbramo, Regs.Ass, rAddress, Regs.Ass);
                yield return InstructionEncoder.Encode(McInstructionType.Set, Regs.Zero, rDest, Regs.Ass);
            } else {
                yield return InstructionEncoder.Encode(McInstructionType.Lbramo, rDest, rAddress, Regs.Ass);
            }
        }
    }
}
