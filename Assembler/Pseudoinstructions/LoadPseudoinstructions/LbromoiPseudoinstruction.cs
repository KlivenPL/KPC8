...
namespace Assembler.Pseudoinstructions.LoadPseudoinstructions {
    ...
    /// AddwI $rDest, #value16
    /// </summary>
    class LbromoiPseudoinstruction : PseudoinstructionBase {
        public override PseudoinstructionType Type => PseudoinstructionType.LbromoI;
        ...
        protected override IEnumerable<IEnumerable<BitArray>> ParseInner(TokenReader reader) {
            ...
            if (DoesDestRegisterViolateDefaultRestrictions(rDestToken)) {
                yield return InstructionEncoder.Encode(McInstructionType.Lbromo, Regs.Ass, rAddress, Regs.Ass);
                yield return InstructionEncoder.Encode(McInstructionType.Set, Regs.Zero, rDest, Regs.Ass);
            } else {
                yield return InstructionEncoder.Encode(McInstructionType.Lbromo, rDest, rAddress, Regs.Ass);
            }
        }
    }
}
