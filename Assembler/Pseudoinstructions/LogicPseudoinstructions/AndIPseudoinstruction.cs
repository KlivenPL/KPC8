...
namespace Assembler.Pseudoinstructions.LogicPseudoinstructions {
    ...
    /// Ori $rA, #value8
    /// </summary>
    class AndiPseudoinstruction : PseudoinstructionBase {
        public override PseudoinstructionType Type => PseudoinstructionType.AndI;
        ...
        protected override IEnumerable<IEnumerable<BitArray>> ParseInner(TokenReader reader) {
            ...
            if (DoesDestRegisterViolateDefaultRestrictions(rAToken)) {
                yield return InstructionEncoder.Encode(McInstructionType.And, Regs.Ass, rA, Regs.Ass);
                yield return InstructionEncoder.Encode(McInstructionType.Set, Regs.Zero, rA, Regs.Ass);
            } else {
                yield return InstructionEncoder.Encode(McInstructionType.And, rA, rA, Regs.Ass);
            }
        }
    }
}
